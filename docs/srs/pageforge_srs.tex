...
\begin{titlepage}
    ...
    {\Huge \textbf{Software Requirements Specification and Design Document}}\\
    \vspace{0.5cm}
    {\Huge \textbf{for}}\\
    \vspace{0.5cm}
    {\Huge \textbf{PageForge}}\\
    
    \vspace{1.5cm}
    {\Large \textbf{Version 2.0 Approved}}\\
    \vspace{2cm}
    
    % Centered Submitted By Section
    {\Large \textbf{Submitted by:}}\\
    \vspace{0.5cm}
    {\large Aaryan Gupta}\\
    {\large Praveen Parakh}\\
    {\large Ayush Kukreti}\\
    {\large Abhishek Kumar}\\
    {\large Shubham Kumar}\\
    
    \vspace{1.5cm}
    
    % Centered Submitted To Section
...
\hline
\textbf{S.No} & \textbf{Name} & \textbf{Date} & \textbf{Reason For Changes} & \textbf{Version} \\
\hline
...
\begin{itemize}
    \item \textbf{Developers:} Sections 3--6 for external interfaces, system features, design, and architecture. Start with Section 5.1 (System Architecture) for an overview.
    \item \textbf{Project Managers:} Sections 1--2 for scope and overview, Section 7 for traceability.
    \item \textbf{Testers:} Sections 4.1--4.11 for feature requirements and stimulus/response sequences, Section 5.2 for sequence diagrams.
    \item \textbf{Evaluators:} Section 5 (Software Design) for architecture, class diagrams, sequence diagrams, database schema, and security design. Section 7 for requirement-to-design traceability.
...
\begin{itemize}
    ...
    \item \textbf{Package Manager:} pnpm 9+ with workspace support.
...
\begin{description}
    \item[UI-01] The web dashboard shall be a Next.js 14 App Router application served on port 3000, using TailwindCSS for styling with a dark mode default theme.
    \item[UI-02] The dashboard shall provide the following primary screens: Login, Register (with OTP verification step), Forgot Password (three-step flow), Dashboard (project list), Project Detail (with Overview, Deployments, Environment, Domains, and Settings tabs), New Project Wizard (three-step), Deployment Detail (with build log viewer), and Settings (GitHub integration).
    \item[UI-03] All forms shall display inline validation errors, loading spinners during submission, and toast notifications for success/failure outcomes.
    \item[UI-04] The OTP verification screens shall present six individual digit input boxes with auto-advance on input, paste support for the full code, and auto-submit when all digits are filled.
    \item[UI-05] The build log viewer shall display real-time streaming output with visual distinction between \texttt{stdout} (default), \texttt{stderr} (red), and \texttt{system} (blue/muted) messages, with auto-scroll to the latest entry.
    \item[UI-06] All pages shall implement proper loading, error, and empty states. The dashboard shall display a centered empty-state prompt when no projects exist.
    \item[UI-07] The interface shall be usable on modern desktop browsers (Chrome 90+, Firefox 90+, Safari 15+, Edge 90+). Mobile-responsive layout is not a v1.0 requirement.
\end{description}

\subsection{Hardware Interfaces}
PageForge does not interface directly with any specialized hardware devices. The system requires:
\begin{itemize}
    \item A Linux server with network access (for serving the dashboard, APIs, and deployed sites).
    \item Access to the Docker Engine socket (\texttt{/var/run/docker.sock}) for creating and managing build containers.
    \item Sufficient disk space for MongoDB data, Redis persistence, MinIO artifact storage, and Docker images.
\end{itemize}

\subsection{Software Interfaces}
\begin{description}
    \item[SI-01 MongoDB 7.0+:] Primary data store for users, projects, and deployments. Accessed via Mongoose ODM over the MongoDB wire protocol (default port 27017). Used for persistent storage of all application state and build log history.
    \item[SI-02 Redis 7.0+:] Used for three purposes: (1) BullMQ job queue for build jobs, (2) Pub/Sub channels for real-time build log streaming (\texttt{build-logs:\{deploymentId\}}), and (3) temporary storage of OTPs (5-minute TTL) and pending registration data (10-minute TTL). Accessed via \texttt{ioredis} client over the Redis protocol (default port 6379).
    \item[SI-03 MinIO (S3-compatible):] Object storage for build artifacts and uploaded ZIP files. Accessed via the MinIO JavaScript SDK using S3-compatible API (default port 9000). Artifacts are stored under \texttt{artifacts/\{deploymentId\}/} and uploads under \texttt{uploads/\{slug\}/\{filename\}}.
    \item[SI-04 Docker Engine 24+:] Build execution environment. Accessed via the Docker Engine API over the Unix socket (\texttt{/var/run/docker.sock}) using the \texttt{dockerode} library. The system creates ephemeral containers from the \texttt{node:20-alpine} image, executes build commands, extracts artifacts, and destroys containers.
    \item[SI-05 Caddy 2.x:] Reverse proxy and static file server. Configured dynamically via the Caddy JSON Admin API (default \texttt{http://localhost:2019}). PageForge adds and removes routes to map project subdomains and custom domains to MinIO artifact paths.
    \item[SI-06 GitHub API v3 (REST):] Used for two integrations: (1) OAuth App flow for user authorization (\texttt{github.com/login/oauth/authorize} and \texttt{github.com/login/oauth/access\_token}), and (2) fetching user profile and repository listings via \texttt{api.github.com}. Requires a registered GitHub OAuth App with \texttt{repo} scope.
    \item[SI-07 SMTP Server:] Used for sending OTP emails during registration and password reset. Accessed via \texttt{nodemailer} over SMTP protocol. Configurable via \texttt{SMTP\_HOST}, \texttt{SMTP\_PORT}, \texttt{SMTP\_USER}, \texttt{SMTP\_PASS}, and \texttt{SMTP\_FROM} environment variables. Currently configured with Brevo (Sendinblue) SMTP relay.
    \item[SI-08 Node.js 20+ Runtime:] Both the web application and worker process run on Node.js. The web app uses the Next.js custom server (\texttt{server.ts}) and the worker runs as a standalone Node.js process.
\end{description}

\subsection{Communications Interfaces}
\begin{description}
    \item[CI-01 HTTP/HTTPS:] All client-to-server communication uses HTTP (development) or HTTPS (production via Caddy TLS termination). The REST API follows standard HTTP methods (GET, POST, PATCH, PUT, DELETE) with JSON request/response bodies.
    \item[CI-02 WebSocket:] Real-time build log streaming uses WebSocket connections at \texttt{ws://\{host\}/ws/logs/\{deploymentId\}}. The WebSocket server runs alongside the Next.js custom server and bridges Redis Pub/Sub messages to connected browser clients.
    \item[CI-03 DNS:] Custom domain verification requires CNAME DNS record resolution. The system resolves CNAME records for user-provided domains and compares against the expected target (\texttt{\{slug\}.\{PAGEFORGE\_DOMAIN\}}).
    \item[CI-04 Cloudflare Tunnel (Optional):] For public access without port forwarding, the system supports Cloudflare Tunnel (\texttt{cloudflared}) to expose the server securely. This is an optional deployment configuration, not a system requirement.
\end{description}
...
Allows new users to create an account with name, email, and password. Email address is verified via OTP before account creation (see Section 4.3). \textbf{Priority: High.}
...
\begin{description}
    ...
    \item[FR-03] The system shall reject registration if the email is already registered (HTTP 409). Note: this intentionally reveals email existence to provide clear user feedback; the anti-enumeration requirement FR-91 applies only to the password reset flow.
    \item[FR-04] The system shall hash passwords using bcrypt with 12 salt rounds before storage.
    \item[FR-05] The system shall require email OTP verification before finalizing account creation (see Section 4.3).
...
\begin{description}
    ...
    \item[FR-91] The system shall not reveal whether an email is registered or not during the password reset flow (to prevent user enumeration via the forgot-password endpoint).
    ...
    \item[FR-97] The system shall require the user to log in again after a successful password reset. Since JWT sessions are stateless, existing tokens remain valid until their natural 30-day expiry; the password change itself prevents further logins with the old password.
...
\begin{center}\textbf{SD-01: User Registration}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-02: User Login (Credentials)}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-03: Email OTP Verification (Registration \& Password Reset)}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-04: Create New Project}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-05: Trigger Deployment and Build Process}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-06: GitHub OAuth Connection}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-07: Custom Domain Management}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-08: ZIP Upload and Deploy}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-09: Environment Variable Management}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\needspace{8\baselineskip}
\begin{center}\textbf{SD-10: Authentication Middleware Flow}\end{center}
\vspace{-0.5em}
\begin{figure}[H]
    \centering
...
\begin{description}
    \item[NFR-01] Dashboard pages shall load promptly without noticeable delay on a standard broadband connection.
    \item[NFR-02] API responses for CRUD operations shall feel near-instantaneous to the user under normal load.
    \item[NFR-03] Build log streaming shall deliver output to the browser with minimal perceptible latency so the user experience feels real-time.
    \item[NFR-04] The system shall support concurrent builds limited by the host machine's Docker resource availability (CPU, memory, disk I/O).
    \item[NFR-05] Build start-up time shall be significantly faster than VM-based platforms (such as Cloudflare Pages) due to the use of lightweight Docker containers.
    \item[NFR-06] GitHub repository listing shall support pagination to remain responsive regardless of how many repositories the user has.
\end{description}
...
\begin{description}
    ...
    \item[NFR-19] \textbf{Usability:} Dark mode default, consistent loading/error/empty states on all pages, desktop-optimized layout. Mobile-responsive layout is deferred to a future version.

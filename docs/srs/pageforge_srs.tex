...
\begin{itemize}
    ...
    \item Multi-user authentication with email/password credentials and email OTP login.
...
\begin{enumerate}
    ...
    \item MinIO JavaScript Client: \url{https://min.io/docs/minio/linux/developers/javascript/minio-javascript.html}
    \item GitHub OAuth Documentation: \url{https://docs.github.com/en/apps/oauth-apps}
...
\begin{itemize}
    \item \textbf{User Authentication:} Registration, login (email/password and OTP), session management, logout.
...
\begin{itemize}
    ...
    \item \textbf{Single Server:} Designed for single-server deployment; no horizontal scaling or clustering support.
...
Allows new users to create an account with name, email, and password. \textbf{Priority: High.}
...
\begin{description}
    ...
    \item[FR-05] The system shall store the user in MongoDB with \texttt{createdAt} and \texttt{updatedAt} timestamps.
    \item[FR-06] The system shall return HTTP 201 on successful registration.
...
\subsection{User Login (Email OTP)}
\subsubsection{Description and Priority}
Allows users to authenticate via a one-time password sent to their email. \textbf{Priority: Medium.}

\subsubsection{Stimulus/Response Sequences}
\begin{enumerate}
    \item User navigates to \texttt{/login} and selects ``Login with OTP''.
    \item User enters email address.
    \item System sends 6-digit OTP to the email.
    \item User enters OTP code.
    \item System verifies OTP and issues JWT session.
\end{enumerate}

\subsubsection{Functional Requirements}
\begin{description}
    \item[FR-15] The system shall provide an OTP login option on the login page.
    \item[FR-16] The system shall generate a cryptographically random 6-digit OTP.
    \item[FR-17] The system shall send the OTP to the user's registered email address.
    ...
    \item[FR-21] The system shall issue a JWT session token upon successful OTP verification.
    \item[FR-22] The system shall return HTTP 404 if the email is not registered.
\end{description}
...
\begin{description}
    ...
    \item[FR-57] The worker shall publish log entries to Redis Pub/Sub channel \texttt{build-logs:\{deploymentId\}}.
    ...
    \item[FR-60] The worker shall upload extracted artifacts to MinIO at \texttt{artifacts/\{deploymentId\}/}.
    \item[FR-61] The worker shall configure a Caddy route mapping \texttt{\{slug\}.\{domain\}} to the MinIO artifact path.
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{diagrams/class_diagram.png}
...
\begin{itemize}
    ...
    \item \textbf{OTPService} manages OTP generation, verification, and email delivery for passwordless login.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/seq_register.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/seq_login.png}
    \caption{Sequence Diagram: User Login with Credentials (D-SQ-02)}
    \label{fig:seq_login}
\end{figure}

\paragraph{SD-03: User Login (Email OTP)}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/seq_otp_login.png}
    \caption{Sequence Diagram: Email OTP Login (D-SQ-03)}
    \label{fig:seq_otp}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{diagrams/seq_create_project.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{diagrams/seq_deploy.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/seq_github_oauth.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{diagrams/seq_domain.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/seq_upload_zip.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{diagrams/seq_env_vars.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{diagrams/seq_middleware.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{diagrams/database_schema.png}
...
\begin{enumerate}
    ...
    \item \textbf{OTP-Based Authentication:}
    \begin{itemize}
...
\hline
UC-03 & User Login (Email OTP) & D-CL-01, D-SQ-03, D-SEC-01 & Class Diagram, Sequence Diagram, Security Design \\
\hline
...
\hline
FR-15 to FR-22 & Login (OTP) Reqs & D-SQ-03, D-SEC-01 & Sequence Diagram, Security Design \\
\hline
...
All analysis models are included as PlantUML-generated diagrams throughout Section 4 (Software Design):
\begin{itemize}
    \item \textbf{System Architecture Diagram} --- Figure~\ref{fig:architecture}
    \item \textbf{Class Diagram} --- Figure~\ref{fig:class}
    \item \textbf{Database ER Diagram} --- Figure~\ref{fig:db}
    \item \textbf{Sequence Diagrams} --- Figures~\ref{fig:seq_register} through~\ref{fig:seq_mw}
\end{itemize}

PlantUML source files are located in \texttt{docs/srs/diagrams/} directory.
